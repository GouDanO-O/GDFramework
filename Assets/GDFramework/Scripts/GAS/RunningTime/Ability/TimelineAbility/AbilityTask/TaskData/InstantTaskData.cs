...
using TypeUtil = GDFramework_Core.GAS.General.Util.TypeUtil;
...
namespace GDFramework_Core.GAS.RunningTime.Ability.TimelineAbility.AbilityTask.TaskData
{
    ...
    public class InstantTaskData : AbilityTaskData
    {
        ...
        public override AbilityTaskBase Load()
        {
            ...
            var dataType = string.IsNullOrEmpty(TaskData.Type)
                ? typeof(DefaultInstantAbilityTask).FullName
                : TaskData.Type;
        ...
        {
            get { return InstantTaskSonTypes.Select(sonType => sonType.FullName).ToList(); }
        }
