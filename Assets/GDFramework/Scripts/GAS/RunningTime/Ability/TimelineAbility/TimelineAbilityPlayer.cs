...
namespace GDFramework_Core.GAS.RunningTime.Ability.TimelineAbility
{
    ...
    public class TimelineAbilityPlayer<T> where T : AbstractAbility
    {
        ...
        private void Cache_InstantCues()
        {
            ...
            foreach (var trackData in AbilityAsset.InstantCues) _cacheInstantCues.AddRange(trackData.markEvents);
        ...
        {
            foreach (var runtimeBuffClip in _cacheBuffGameplayEffectTrack) runtimeBuffClip.buffSpec = null;
        }
        ...
        private void TickFrame_DurationalGameplayCues(int frame)
        {
            foreach (var cueClip in _cacheDurationalCueTrack)
            {
                ...
                if (frame >= cueClip.startFrame && frame <= cueClip.endFrame) cueClip.cueSpec.OnTick();
