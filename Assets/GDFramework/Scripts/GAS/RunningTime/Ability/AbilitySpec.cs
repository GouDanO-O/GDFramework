...
namespace GDFramework_Core.GAS.RunningTime.Ability
{
    public abstract class AbilitySpec
    {
        ...
        protected event Action<EAbilityActivateResult> _onActivateResult;
        protected event Action _onEndAbility;
        protected event Action _onCancelAbility;

        public void RegisterActivateResult(Action<EAbilityActivateResult> onActivateResult)
        {
            _onActivateResult += onActivateResult;
        }

        public void UnregisterActivateResult(Action<EAbilityActivateResult> onActivateResult)
        {
            _onActivateResult -= onActivateResult;
        }
        ...
        public virtual EAbilityActivateResult CanActivate()
        {
            if (IsActive) return EAbilityActivateResult.FailHasActivated;
            if (!CheckGameplayTagsValidTpActivate()) return EAbilityActivateResult.FailTagRequirement;
            if (!CheckCost()) return EAbilityActivateResult.FailCost;
            if (CheckCooldown().TimeRemaining > 0) return EAbilityActivateResult.FailCooldown;
        ...
        public virtual bool TryActivateAbility(params object[] args)
        {
            ...
            var success = result == EAbilityActivateResult.Success;
            if (success)
            {
                IsActive = true;
                ActiveCount++;
