...
namespace GDFramework_Core.GAS.RunningTime.Attribute
{
    public class AttributeAggregator
    {
        ...
        /// </summary>
        private List<Tuple<GameplayEffectSpec, GameplayEffectModifier>> _modifierCache = new();
        ...
        private void UpdateCurrentValueWhenBaseValueIsDirty(AttributeBase attribute, float oldBaseValue,
            float newBaseValue)
        {
        ...
        private void TryUnregisterAttributeChangedListen(GameplayEffectSpec ge, GameplayEffectModifier modifier)
        {
                ...
                mmc.captureType == AttributeBasedModCalculation.GEAttributeCaptureType.Track)
            {
                ...
                {
                    if (ge.Owner != null)
                        ge.Owner.AttributeSetContainer.AttributeSetDict[mmc.attributeSetName][mmc.attributeShortName]
                            .UnregisterPostCurrentValueChange(OnAttributeChanged);
                }
                else
                {
                    if (ge.Source != null)
                        ge.Source.AttributeSetContainer.AttributeSetDict[mmc.attributeSetName][mmc.attributeShortName]
                            .UnregisterPostCurrentValueChange(OnAttributeChanged);
                }
            }
        }
