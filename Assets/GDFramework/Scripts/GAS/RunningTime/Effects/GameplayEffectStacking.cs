...
namespace GDFramework_Core.GAS.RunningTime.Effects
{
    ...
    public struct GameplayEffectStacking
    {
        ...
        public void SetStackingCodeName(string stackingCodeName)
        {
            ...
            stackingHashCode = stackingCodeName?.GetHashCode() ?? 0; // 兼容旧的SO数据
        }
    ...
    public sealed class GameplayEffectStackingConfig
    {
        ...
        [LabelWidth(LABEL_WIDTH)] [VerticalGroup] [LabelText(GasTextDefine.LABEL_GE_STACKING_TYPE)] [EnumToggleButtons]
        public StackingType stackingType;
        ...
        [HideIf("IsNoStacking")]
        [InfoBox(GasTextDefine.LABEL_GE_STACKING_DENY_OVERFLOW_APPLICATION + "为True时多余的Apply不会刷新Duration",
        ...
        public bool IsNoStacking()
        {
            return stackingType == StackingType.None;
        }

        public bool IsNeverRefreshDuration()
        {
            return IsNoStacking() || durationRefreshPolicy == DurationRefreshPolicy.NeverRefresh;
        }

        public bool IsDenyOverflowApplication()
        {
            return !IsNoStacking() && denyOverflowApplication;
        }
