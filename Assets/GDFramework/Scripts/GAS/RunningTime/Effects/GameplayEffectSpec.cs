...
namespace GDFramework_Core.GAS.RunningTime.Effects
{
    ...
    {
        private Dictionary<GameplayTag, float> _valueMapWithTag = new();
        private Dictionary<string, float> _valueMapWithName = new();
        private List<GameplayCueDurationalSpec> _cueDurationalSpecs = new();
...
#pragma warning restore CS0067 // 事件从未使用过
        ...
        public void SetGrantedAbility(GrantedAbilityFromEffect[] grantedAbility)
        {
            ...
            for (var i = 0; i < grantedAbility.Length; i++) GrantedAbilitySpec[i] = grantedAbility[i].CreateSpec(this);
        }
        ...
        public void Apply()
        {
            ...
            if (GameplayEffect.CanRunning(Owner)) Activate();
        }
        ...
        private void TriggerInstantCues(GameplayCueInstant[] cues)
        {
        ...
        {
            SnapshotSourceAttributes = Source.DataSnapshotDict();
            SnapshotTargetAttributes = Source == Owner ? SnapshotSourceAttributes : Owner.DataSnapshotDict();
        }
            ...
            else
            {
                ...
                if (Stacking.durationRefreshPolicy == DurationRefreshPolicy.RefreshOnSuccessfulApplication)
                {
                    if (Stacking.denyOverflowApplication)
                    {
                        ...
                        if (Stacking.clearStackOnOverflow) RemoveSelf();
                    }
                    else
                    {
                        RefreshDuration();
