...
namespace GDFramework.GAS.General.Util
{
    ...
    public sealed class PriorityValue<T> : IDisposable
    {
        ...
        {
            _defaultData = new Data(DefaultKey, defaultPriority, defaultValue);
        ...
        public T AddOrSet(string key, int priority, T value)
        {
            lock (_lock)
            {
                ...
                if (index >= 0)
                {
                    ...
                    {
                        _datas[index] = new Data(key, priority, value);
                    }
                    else
                    {
        ...
        public T SetDefault(int priority, T value)
        {
            lock (_lock)
            {
                ...
                {
                    _defaultData = new Data(DefaultKey, priority, value);
                    Refresh();
                }
        ...
        private void Refresh()
        {
            ...
            {
                var data = _datas[0];
                if (data.Priority > _defaultData.Priority) _currentData = data;
            }
    ...
    public sealed class PriorityValueToggle<T> : IDisposable
    {
        ...
        public int Priority
        {
            ...
            set
            {
                ...
                {
                    _data = new PriorityValue<T>.Data(Key, value, Value);
                    if (IsEnabled) PriorityValue?.AddOrSet(Key, Priority, Value);
                }
            }
        }
        ...
        public T Value
        {
            ...
            set
            {
                ...
                {
                    _data = new PriorityValue<T>.Data(Key, Priority, value);
                    if (IsEnabled) PriorityValue?.AddOrSet(Key, Priority, Value);
                }
            }
        }
