...
namespace GDFramework.World
{
    public class AABB_Check : IEquatable<AABB_Check>
    {
        ...
        public void AlignToGrid(float gridSize)
        {
            ...
            Y = Mathf.Floor(Y / gridSize) * gridSize;
            W = Mathf.Ceil(W / gridSize) * gridSize;
            H = Mathf.Ceil(H / gridSize) * gridSize;
        }
        ...
        public void Reorder()
        {
            var minX = Mathf.Min(Left, Right);
            var maxX = Mathf.Max(Left, Right);
        ...
        public static void GetOrthorCameraAABB(Camera cam, ref AABB_Check aabb,
            float h_padding = DefaultGetCameraFrustumToAABBHorPadding)
        {
        ...
        public static void GetFrustumCameraAABBs(Camera cam, List<AABB_Check> aabbs,
            int level = DefaultGetCameraFrustumToAABBLevel, float h_padding = DefaultGetCameraFrustumToAABBHorPadding)
        {
            // 计算椎体分段包围盒
