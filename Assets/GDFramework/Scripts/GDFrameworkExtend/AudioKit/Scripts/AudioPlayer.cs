...
namespace QFramework
{
    public class AudioPlayer : IPoolable, IPoolType, IAudioKitOnFinish
    {
        ...
        {
            if (clip == null || mName == name) return;

            if (mAudioSource == null) mAudioSource = root.AddComponent<AudioSource>();
        ...
        {
            if (mAudioSource) mAudioSource.volume = mVolume * mVolumeScale;
        }
        ...
        private void OnResumeTimeTick(int repeatCount)
        {
            ...
            if (IsLoop) mTimeItem = Timer.Instance.Post2Scale(OnSoundPlayFinish, mAudioClip.length, -1);
        }
