...
namespace GDFrameworkExtend._CoreKit.ActionKit
{
    internal class DelayFrame : IAction
    {
        ...
        private static SimpleObjectPool<DelayFrame> mSimpleObjectPool = new(() => new DelayFrame(), null, 10);

        private System.Action mOnDelayFinish;

        public static DelayFrame Allocate(int frameCount, System.Action onDelayFinish = null)
        {
    ...
    public static class DelayFrameExtension
    {
        ...
        {
            return self.Append(Action.DelayFrame.Allocate(frameCount));
        }

        public static ISequence NextFrame(this ISequence self)
        {
            return self.Append(Action.DelayFrame.Allocate(1));
        }
    }
}