...
namespace GDFrameworkExtend._CoreKit.ActionKit
{
    internal class MonoUpdateActionExecutor : MonoBehaviour, IActionExecutor
    {
        ...
        private List<ActionTask> mPrepareExecutionActions = new();

        private Dictionary<IAction, ActionTask> mExecutingActions = new();

        private static SimpleObjectPool<ActionTask> mActionTaskPool = new(
            () => new ActionTask(), (task) =>
            {
                task.Action = null;
        ...
        private List<IActionController> mToActionRemove = new();
