...
namespace GDFrameworkExtend._CoreKit.ActionKit
{
    ...
    public class Repeat : IRepeat
    {
        ...
        private static SimpleObjectPool<Repeat> mSimpleObjectPool = new(() => new Repeat(), null, 5);
    ...
    public static class RepeatExtension
    {
        ...
        {
            var repeat = Action.Repeat.Allocate();
        ...
        {
            var repeat = Action.Repeat.Allocate(repeatCount);
            repeatSetting(repeat);
            return self.Append(repeat);
        }
    }
