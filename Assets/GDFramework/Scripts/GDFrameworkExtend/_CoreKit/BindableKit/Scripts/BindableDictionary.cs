...
namespace QFramework
{
        ...
        IDictionary, ISerializable, IDeserializationCallback
    {
        ...
        {
            mInner = new Dictionary<TKey, TValue>();
        }

        public BindableDictionary(IEqualityComparer<TKey> comparer)
        {
            mInner = new Dictionary<TKey, TValue>(comparer);
        }

        public BindableDictionary(Dictionary<TKey, TValue> innerDictionary)
        {
            mInner = innerDictionary;
        }
        ...
        {
            return mInner.TryGetValue(key, out value);
        }

        public Dictionary<TKey, TValue>.Enumerator GetEnumerator()
        {
            return mInner.GetEnumerator();
        }


        [NonSerialized] private EasyEvent<int> mOnCountChanged;
        public EasyEvent<int> OnCountChanged => mOnCountChanged ?? (mOnCountChanged = new EasyEvent<int>());


        [NonSerialized] private EasyEvent mOnClear;
        public EasyEvent OnClear => mOnClear ?? (mOnClear = new EasyEvent());

        [NonSerialized] private EasyEvent<TKey, TValue> mOnAdd;
        public EasyEvent<TKey, TValue> OnAdd => mOnAdd ?? (mOnAdd = new EasyEvent<TKey, TValue>());

        [NonSerialized] private EasyEvent<TKey, TValue> mOnRemove;
        public EasyEvent<TKey, TValue> OnRemove => mOnRemove ?? (mOnRemove = new EasyEvent<TKey, TValue>());

        [NonSerialized] private EasyEvent<TKey, TValue, TValue> mOnReplace;
        ...
        public EasyEvent<TKey, TValue, TValue> OnReplace =>
            mOnReplace ?? (mOnReplace = new EasyEvent<TKey, TValue, TValue>());
        ...
        bool ICollection<KeyValuePair<TKey, TValue>>.IsReadOnly =>
            ((ICollection<KeyValuePair<TKey, TValue>>)mInner).IsReadOnly;
        ...
        {
            return ((IDictionary)mInner).Contains(key);
        }

        void ICollection.CopyTo(Array array, int index)
        {
            ((IDictionary)mInner).CopyTo(array, index);
        }

        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ((ISerializable)mInner).GetObjectData(info, context);
        }

        public void OnDeserialization(object sender)
        {
            ((IDeserializationCallback)mInner).OnDeserialization(sender);
        }
        ...
        {
            return ((ICollection<KeyValuePair<TKey, TValue>>)mInner).Contains(item);
        }

        void ICollection<KeyValuePair<TKey, TValue>>.CopyTo(KeyValuePair<TKey, TValue>[] array, int arrayIndex)
        {
            ((ICollection<KeyValuePair<TKey, TValue>>)mInner).CopyTo(array, arrayIndex);
        }

        IEnumerator<KeyValuePair<TKey, TValue>> IEnumerable<KeyValuePair<TKey, TValue>>.GetEnumerator()
        {
            return ((ICollection<KeyValuePair<TKey, TValue>>)mInner).GetEnumerator();
        }
        ...
        {
            return ((IDictionary)mInner).GetEnumerator();
        }
