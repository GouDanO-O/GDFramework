...
namespace QFramework
{
    ...
    public class ViewControllerInspector : Editor
    {
        ...
        private ViewControllerInspectorLocale mLocaleText = new();
        ...
        private void OnEnable()
        {
            ...
            mViewControllerTypeMenus = mViewControllerTypes.Select(t => t.FullName).Append("QFramework.ViewController")
                .ToArray();
        }
        ...
        private static Type[] SearchAllArchitectureTypes()
        {
            ...
            return AppDomain.CurrentDomain.GetAssemblies().Where(a =>
                    !a.FullName.Contains("UnityEngine") && !a.FullName.Contains("Kit") &&
                    !a.FullName.Contains("QFramework"))
        ...
        private static Type[] SearchAllViewControllerTypes()
        {
            ...
            return AppDomain.CurrentDomain.GetAssemblies().Where(a =>
                ...
                .SelectMany(a => a.GetTypes())
                .Where(type =>
                    type.GetAttribute<ViewControllerChildAttribute>() != null &&
                    viewControllerType.IsAssignableFrom(type)).ToArray();
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (mArchitectureTypes.Length > 0)
            {
                ...
                if (index == -1) index = mArchitectureTypeMenus.Length - 1;
            ...
            if (mViewControllerTypes.Length > 0)
            {
                ...
                if (index == -1) index = mViewControllerTypeMenus.Length - 1;
            ...
            else
            {
                if (ViewController.GetType() != typeof(ViewController))
                {
                    var scriptPath = AssetDatabase
                        ...
                        .FirstOrDefault(path =>
                            AssetDatabase.LoadAssetAtPath<MonoScript>(path).GetClass() == ViewController.GetType());

                    if (scriptPath != null) ViewController.ScriptsFolder = scriptPath.GetFolderPath();
                }
            }
