...
namespace KINEMATION.ScriptableWidget
{
    public class ScriptableComponentListWidget
    {
        ...
        private void CopyComponent(Object component)
        {
            ...
            var typeData = JsonUtility.ToJson(component);
        ...
        private void OnContextMenuSelection(object userData, string[] options, int selected)
        {
            ...
            var component = _componentsProperty.GetArrayElementAtIndex(index).objectReferenceValue;
        ...
        private void SetupReorderableList(string targetSerializedPropertyName)
        {
            ...
            _componentsList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                var labelWidth = rect.width * headerSpaceRatio;
                var buttonWidth = rect.width * (1f - headerSpaceRatio);

                var labelRect = new Rect(rect.x, rect.y, labelWidth, singleHeight);
                var buttonRect = new Rect(rect.x + labelWidth, rect.y, buttonWidth, singleHeight);

                var elementName = element.objectReferenceValue.name;
                ...
                {
                    var menuOptions = new GUIContent[]
                    {
                        new("Copy"),
                        new("Paste")
                    };
            ...
            {
                var oldIndexEditor = _editors[oldIndex];
                var newIndexEditor = _editors[newIndex];

                _editors[oldIndex] = newIndexEditor;
                _editors[newIndex] = oldIndexEditor;
            };

            _componentsList.onRemoveCallback = list => { RemoveComponent(list.index); };
        }
            ...
            bool standaloneWindow = true)
        {
            ...
            var arraySize = _componentsProperty.arraySize;
        ...
        public void OnGUI()
        {
            ...
            {
                var count = _componentTypes.Length;

                var menuOptions = new GUIContent[count];
                for (var i = 0; i < count; i++) menuOptions[i] = new GUIContent(_componentTypes[i].Name);

                EditorUtility.DisplayCustomMenu(new Rect(Event.current.mousePosition, Vector2.zero),
                    menuOptions, -1, OnTypeSelected, null);
            }

            if (_useStandaloneWindow || _editorIndex < 0 || _editors.Count == 0) return;
