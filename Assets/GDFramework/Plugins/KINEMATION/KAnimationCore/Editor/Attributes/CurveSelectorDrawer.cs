...
namespace KINEMATION.KAnimationCore.Editor.Attributes
{
    ...
    public class CurveSelectorDrawer : PropertyDrawer
    {
        ...
        private void AddInputNames(ref List<string> options, UserInputConfig config)
        {
            ...
            foreach (var property in config.floatProperties) options.Add($"{property.name} (Input)");
        }

        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            var curveAttribute = attribute as CurveSelectorAttribute;
            if (curveAttribute == null) return;
            ...
            if (rig != null)
            {
                ...
                if (curveAttribute.useInput) AddInputNames(ref options, rig.inputConfig);
            }

            name ??= property;

            var index = options.IndexOf(name.stringValue);

            if (index < 0) index = options.ToList().IndexOf(name.stringValue + " (Animator)");

            if (index < 0) index = options.ToList().IndexOf(name.stringValue + " (Playables)");

            if (index < 0) index = options.ToList().IndexOf(name.stringValue + " (Input)");

            var propertyRect = position;
            propertyRect.height = EditorGUIUtility.singleLineHeight;

            index = EditorGUI.Popup(propertyRect, label.text, index, options.ToArray());
            var selection = index >= 0 ? options[index] : "None";

            if (source != null)
            {
                if (selection.EndsWith("(Animator)")) source.intValue = 0;

                if (selection.EndsWith("(Playables)")) source.intValue = 1;

                if (selection.EndsWith("(Input)")) source.intValue = 2;
            }
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (name == null || mode == null || clampMin == null) return base.GetPropertyHeight(property, label);
