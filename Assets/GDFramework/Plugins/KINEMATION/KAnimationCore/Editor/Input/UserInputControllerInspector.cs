...
namespace KINEMATION.KAnimationCore.Editor.Input
{
    ...
    public class UserInputControllerInspector : UnityEditor.Editor
    {
        ...
        private static bool IsInspectorFocused()
        {
            ...
            if (focusedWindow != null && focusedWindow.GetType().Name == "InspectorWindow") return true;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                var label = property.Item1;
                var value = property.Item2;

                if (value is bool)
                {
                    var toggle = EditorGUILayout.Toggle(label, (bool)value);
                    if (toggle != (bool)value) _controller.SetValue(label, toggle);
                }
                else if (value is int)
                {
                    var integer = EditorGUILayout.IntField(label, (int)value);
                    if (integer != (int)value) _controller.SetValue(label, integer);
                }
                else if (value is float)
                {
                    var floatVal = EditorGUILayout.FloatField(label, (float)value);
                    if (!Mathf.Approximately(floatVal, (float)value)) _controller.SetValue(label, floatVal);
                }
                else if (value is Vector4)
                {
                    var vector4 = EditorGUILayout.Vector4Field(label, (Vector4)value);
