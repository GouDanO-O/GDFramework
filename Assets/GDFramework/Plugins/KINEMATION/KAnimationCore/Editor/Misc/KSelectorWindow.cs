...
namespace KINEMATION.KAnimationCore.Editor.Misc
{
    public class KSelectorWindow : EditorWindow
    {
        ...
        public static void ShowWindow(ref List<string> names, ref List<int> depths, OnTreeItemClicked onClicked,
            OnSelectionChanged onSelectionChanged, bool useSelection, List<int> selection = null,
            string title = "Selection")
        {
            var window = CreateInstance<KSelectorWindow>();
            ...
            var namesAndDepths = new (string, int)[names.Count];
            for (var i = 0; i < names.Count; i++) namesAndDepths[i] = (names[i], depths[i]);
            ...
            if (window._useSelection && selection != null) window._rigTreeWidget.rigTreeView.SetSelection(selection);
