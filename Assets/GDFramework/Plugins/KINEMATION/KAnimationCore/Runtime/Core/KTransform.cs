...
namespace KINEMATION.KAnimationCore.Runtime.Core
{
    ...
    public struct KTransform
    {
        ...
        {
            var outPos = Vector3.Lerp(a.position, b.position, alpha);
            var outRot = Quaternion.Slerp(a.rotation, b.rotation, alpha);
            var outScale = Vector3.Lerp(a.scale, a.scale, alpha);
        ...
        {
            var result = position.Equals(other.position) && rotation.Equals(other.rotation);

            if (useScale) result = result && scale.Equals(other.scale);
        ...
        public Vector3 InverseTransformPoint(Vector3 worldPosition, bool useScale)
        {
            ...
            if (useScale) result = Vector3.Scale(scale, result);
        ...
        {
            var result = Quaternion.Inverse(rotation) * worldDirection;
        ...
        {
            if (useScale) localPosition = Vector3.Scale(scale, localPosition);
        ...
        {
            if (useScale) localDirection = Vector3.Scale(scale, localDirection);
