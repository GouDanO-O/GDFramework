...
namespace KINEMATION.KAnimationCore.Runtime.Core
{
    ...
    public struct VectorCurve
    {
        ...
        public float GetCurveLength()
        {
            ...
            var curveTime = KCurves.GetCurveLength(x);
    ...
    public class KCurves
    {
        public static float GetCurveLength(AnimationCurve curve)
        {
            ...
            if (curve != null) length = curve[curve.length - 1].time;
        ...
        public static float Ease(float a, float b, float alpha, EaseMode ease)
        {
            ...
            if (ease.easeFunc == EEaseFunc.Sine) return EaseSine(a, b, alpha);

            if (ease.easeFunc == EEaseFunc.Cubic) return EaseCubic(a, b, alpha);

            if (ease.easeFunc == EEaseFunc.Custom) return EaseCurve(a, b, alpha, ease.curve);
