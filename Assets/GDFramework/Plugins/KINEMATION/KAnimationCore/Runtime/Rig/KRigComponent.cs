...
namespace KINEMATION.KAnimationCore.Runtime.Rig
{
    ...
    {
        [SerializeField] private List<Transform> hierarchy = new();
...
#if UNITY_EDITOR
        [SerializeField] private List<int> hierarchyDepths = new();

        public bool CompareRig(KRig compareTo)
        {
            if (compareTo == null || hierarchy == null || compareTo.rigHierarchy.Count != hierarchy.Count) return false;

            var count = hierarchy.Count;
            for (var i = 0; i < count; i++)
                if (!compareTo.rigHierarchy[i].name.Equals(hierarchy[i].name))
                    return false;
        ...
        public bool Contains(string entry)
        {
            ...
            foreach (var element in hierarchy) set.Add(element.name);
        ...
        private void TraverseHierarchyByLayer(Transform currentTransform, int depth)
        {
            ...
            foreach (Transform child in currentTransform) TraverseHierarchyByLayer(child, depth + 1);
        }
#endif
        ...
        public void Initialize()
        {
            ...
            var virtualElements = GetComponentsInChildren<KVirtualElement>();

            foreach (var virtualElement in virtualElements) _virtualElements.Add(virtualElement);

            // Map the hierarchy indexes to the element names.
            _hierarchyMap = new Dictionary<string, int>();

            var count = hierarchy.Count;
            for (var i = 0; i < count; i++) _hierarchyMap.TryAdd(hierarchy[i].name, i);
        ...
        {
            foreach (var virtualElement in _virtualElements) virtualElement.Animate();
        }
        ...
        {
            var index = rigElement.index;

            // Invalid index, try to use the element name instead.
            if (index < 0 || index > hierarchy.Count - 1) index = _hierarchyMap[rigElement.name];

            // Total failure, return null.
            if (index < 0 || index > hierarchy.Count - 1) return null;
        ...
        {
            if (_hierarchyMap.TryGetValue(elementName, out var element)) return hierarchy[element];
        ...
        {
            if (elementIndex < 0 || elementIndex > hierarchy.Count - 1) return null;
        ...
        public void CacheHierarchyPose()
        {
            ...
            foreach (var element in hierarchy)
                _cachedHierarchyPose.Add(new KTransform(element,
                    false));
        }
