...
namespace KINEMATION.KAnimationCore.Runtime.Input
{
    public class UserInputController : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            {
                var value = (float)_inputProperties[tuple.Item1];
                ...
                {
                    var alpha = KMath.ExpDecayAlpha(Time.deltaTime, tuple.Item2);
        ...
        public virtual void Initialize()
        {
            ...
            var floatsToInterpolate = new List<(int, float, float)>();
            ...
            if (floatsToInterpolate.Count > 0) _floatsToInterpolate = floatsToInterpolate.ToArray();
        ...
        {
            if (_inputPropertyMap.TryGetValue(propertyName, out var index)) return index;
        ...
        {
            if (propertyIndex < 0 || propertyIndex > _inputProperties.Count - 1) return;

            if (_floatsToInterpolate != null)
            {
                var floatToInterpolateIndex = -1;

                for (var i = 0; i < _floatsToInterpolate.Length; i++)
        ...
        {
            if (propertyIndex < 0 || propertyIndex > _inputProperties.Count - 1) return default;
        ...
        public virtual (string, object)[] GetPropertyBindings()
        {
            ...
            var count = _propertyNames.Count;
