...
namespace KINEMATION.FPSAnimationFramework.Editor.Tools
{
    public class FPSAnimatorWizard : EditorWindow
    {
        ...
        private T InitializeComponent<T>(GameObject parent) where T : Behaviour
        {
            ...
            if (component == null) component = parent.AddComponent<T>();
        ...
        {
            var transform = parent.Find(objectName);
            if (transform == null)
            {
        ...
        private void SetupCharacter()
        {
            ...
            var camera = head.gameObject.GetComponentInChildren<Camera>();
            if (camera == null)
            {
            ...
            var path = $"{KEditorUtility.GetProjectWindowFolder()}/{character.name}_FPSAnimator";
            ...
            if (_rigAsset == null)
            {
                ...
                var filePath = AssetDatabase.GenerateUniqueAssetPath($"{path}/Rig_{character.name}.asset");
        ...
        {
            var allowSetup = RenderTransform(ref root, "Root")
            ...
            animatorController = (RuntimeAnimatorController)EditorGUILayout.ObjectField("Animator Controller",
                animatorController,
