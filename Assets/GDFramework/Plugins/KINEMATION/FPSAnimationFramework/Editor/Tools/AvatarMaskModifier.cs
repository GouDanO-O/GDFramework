...
namespace KINEMATION.FPSAnimationFramework.Editor.Tools
{
    public class AvatarMaskModifier : IEditorTool
    {
        ...
        public void Render()
        {
            ...
            {
                for (var i = _maskToModify.transformCount - 1; i >= 0; i--)
                ...
                var path = _maskToModify.GetTransformPath(_maskToModify.transformCount - 1);
                var array = path.Split("/");
                var rootIndex = Array.IndexOf(array, _root.name);
