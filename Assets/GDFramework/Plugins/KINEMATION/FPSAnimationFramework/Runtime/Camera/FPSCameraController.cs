...
namespace KINEMATION.FPSAnimationFramework.Runtime.Camera
{
    public class FPSCameraController : MonoBehaviour
    {
        ...
        protected virtual void UpdateCameraShake()
        {
            ...
            var length = _activeShake.shakeCurve.GetCurveLength();
            _cameraShakePlayback += Time.deltaTime * _activeShake.playRate;
            _cameraShakePlayback = Mathf.Clamp(_cameraShakePlayback, 0f, length);

            var alpha = KMath.ExpDecayAlpha(_activeShake.smoothSpeed, Time.deltaTime);
            if (!KAnimationMath.IsWeightRelevant(alpha)) alpha = 1f;

            var target = _activeShake.shakeCurve.GetValue(_cameraShakePlayback);
        ...
        protected virtual void UpdateCameraAnimation()
        {
            ...
            targetAnimation.rotation =
                Quaternion.Euler(_cameraAnimationAsset.rotation.GetValue(_animationPlayback) * scale);
