...
namespace KINEMATION.FPSAnimationFramework.Runtime.Recoil
{
    ...
    public class RecoilPattern : MonoBehaviour
    {
        ...
        {
            var multiplier = 1f;
            var isOpposite = recoil * compensation <= 0f;
        ...
        private void Update()
        {
            ...
            if (_isFiring)
            {
                ...
                var deltaInput = _userInputController.GetValue<Vector4>(_deltaLookInputPropertyIndex);
                _compensation.x += deltaInput.x;
                _compensation.y += deltaInput.y;
            }

            var alpha = KMath.ExpDecayAlpha(recoilSettings.horizontalSmoothing, Time.deltaTime);
        ...
        public void OnFireStart()
        {
            ...
            if (!_isFiring) _compensation = _accumulatedRecoil = Vector2.zero;
