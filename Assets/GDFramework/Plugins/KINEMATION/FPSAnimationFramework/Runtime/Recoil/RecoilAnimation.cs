...
namespace KINEMATION.FPSAnimationFramework.Runtime.Recoil
{
    ...
    public class RecoilAnimation : MonoBehaviour
    {
        ...
        public void Play()
        {
            ...
            for (var i = 0; i < _stateMachine.Count; i++)
        ...
        {
            var pitch = Random.Range(RecoilData.pitch.x, RecoilData.pitch.y);
            var yawMin = Random.Range(RecoilData.yaw.x, RecoilData.yaw.y);
            var yawMax = Random.Range(RecoilData.yaw.z, RecoilData.yaw.w);

            var yaw = Random.value >= 0.5f ? yawMax : yawMin;

            var rollMin = Random.Range(RecoilData.roll.x, RecoilData.roll.y);
            var rollMax = Random.Range(RecoilData.roll.z, RecoilData.roll.w);

            var roll = Random.value >= 0.5f ? rollMax : rollMin;

            roll = _targetRot.z * roll > 0f && RecoilData.smoothRoll ? -roll : roll;

            var kick = Random.Range(RecoilData.kickback.x, RecoilData.kickback.y);
            var kickRight = Random.Range(RecoilData.kickRight.x, RecoilData.kickRight.y);
            var kickUp = Random.Range(RecoilData.kickUp.x, RecoilData.kickUp.y);
        ...
        private void UpdateSolver()
        {
            ...
            var lastPlayback = _playBack - Time.deltaTime * RecoilData.playRate;
            lastPlayback = Mathf.Max(lastPlayback, 0f);

            var alpha = _tempRotCurve.GetValue(_playBack);
            var lastAlpha = _tempRotCurve.GetValue(lastPlayback);
        ...
        private void ApplySmoothing()
        {
            if (_enableSmoothing)
            {
                var lerped = _smoothRotOut;

                var smooth = RecoilData.smoothRot;
        ...
        {
            _noiseTarget.x = Mathf.Lerp(_noiseTarget.x, 0f,
                KMath.ExpDecayAlpha(RecoilData.noiseDamp.x, Time.deltaTime));
            _noiseTarget.y = Mathf.Lerp(_noiseTarget.y, 0f,
                KMath.ExpDecayAlpha(RecoilData.noiseDamp.y, Time.deltaTime));
        ...
        private void SetupStateMachine()
        {
            ...
            {
                var timerError = 60f / _fireRate / Time.deltaTime + 1;
            ...
            autoState.onPlay = () =>
            {
                ...
                var bCurvesValid = curves.autoRotCurve.IsValid() && curves.autoLocCurve.IsValid();

                _enableSmoothing = bCurvesValid;
                var correction = 60f / _fireRate;
            ...
            autoState.onStop = () =>
            {
                ...
                var tempRot = _tempRotCurve.GetCurveLength();
                var tempLoc = _tempLocCurve.GetCurveLength();
        ...
        {
            var curveAlpha = rot.GetValue(time);
