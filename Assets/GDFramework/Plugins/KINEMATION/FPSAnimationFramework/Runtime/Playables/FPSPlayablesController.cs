...
namespace KINEMATION.FPSAnimationFramework.Runtime.Playables
{
    ...
    {
        [Header("General Settings")] [HideInInspector]
        public AvatarMask upperBodyMask;

        [SerializeField] [InputProperty] protected string playablesWeightProperty = FPSANames.PlayablesWeight;

        [Header("Editor Animation Preview")] [SerializeField]
        protected AnimationClip animationToPreview;
        ...
        {
            if (!_playableGraph.IsValid() || !_masterMixer.IsValid()) return;
        ...
        {
            if (asset.clip == null) return false;

            var animPlayable = new FPSAnimatorPlayable(_playableGraph, asset.clip, null)
            {
        ...
        public virtual bool PlayAnimation(FPSAnimationAsset asset, float startTime = 0f)
        {
            ...
            var blendTime = asset.blendTime;
            blendTime.startTime = startTime;

            var animPlayable = new FPSAnimatorPlayable(_playableGraph, asset.clip,
            ...
            var overridePlayable = new FPSAnimatorPlayable(_playableGraph, asset.clip, null)
            {
                blendTime = blendTime,
                autoBlendOut = true
            };
            ...
            if (asset.overrideMask != null) _overrideMixer.Play(overridePlayable, asset.overrideMask);
        ...
        public virtual bool InitializeControllerEditor()
        {
            ...
            if (_masterMixer.IsValid()) _masterMixer.Destroy();
        ...
        {
            if (!InitializeControllerEditor()) return;
            ...
            if (animationToPreview != null)
            {
                ...
                if (_masterMixer.GetInput(0).IsValid()) _masterMixer.DisconnectInput(0);
        ...
        {
            if (!_playableGraph.IsPlaying()) EditorApplication.update -= LoopEditorPreview;
        ...
        public virtual void StopEditorPreview()
        {
            ...
            if (defaultPose != null) defaultPose.SampleAnimation(gameObject, 0f);
        }
#endif
    }
}