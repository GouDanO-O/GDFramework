...
namespace KINEMATION.FPSAnimationFramework.Runtime.Playables
{
    ...
    {
        [FormerlySerializedAs("Name")] [CurveSelector(false, true, false)]
        public string name;
    ...
    public struct FPSAnimatorMixer
    {
        ...
        public FPSAnimatorMixer(PlayableGraph graph, int inputCount, int layerLevel = 0)
        {
            ...
            for (var i = 0; i < inputCount; i++) _playables.Add(new FPSAnimatorPlayable());
        ...
        public void Stop(float blendOutTime)
        {
            ...
            for (var i = _layerLevel; i < _activeIndex; i++)
            {
        ...
        {
            if (_blendedCurves == null || !_blendedCurves.ContainsKey(curveName) || !_isMixerActive) return 0f;
        ...
        {
            // Inactive curve
            if (blendedAnimCurve.curve == null) return Mathf.Lerp(blendedAnimCurve.valueCache, 0f, BlendInWeight);

            var rawValue = blendedAnimCurve.curve.Evaluate(GetNormalizedTime());

            var blendedValue = Mathf.Lerp(blendedAnimCurve.valueCache, rawValue, BlendInWeight);
        ...
        private void AddUserCurves(AnimCurve[] newCurves)
        {
            ...
            if (newCurves != null) userCurveSet = newCurves.ToDictionary(curve => curve.name);

            foreach (var curve in blendedCurves)
            {
                // Cache all blended curves.
                var animCurve = curve.Value;
            ...
            // Add new curves.
            foreach (var userCurve in userCurveSet) _blendedCurves.Add(userCurve.Key, userCurve.Value);
        }
        ...
        private void UpdateActiveIndex()
        {
            ...
            // Reconnect
            for (var i = _layerLevel; i < mixer.GetInputCount() - 1; i++)
            {
                var inputWeight = mixer.GetInputWeight(i + 1);
                var clip = _playables[i + 1 - _layerLevel];
        ...
        private void BlendOutInactive()
        {
            for (var i = _layerLevel; i < _activeIndex; i++)
            {
                ...
                var weight = _playables[i - _layerLevel].cachedWeight;
            ...
            foreach (var curve in curves)
                if (curve.Value.curve == null)
                    _blendedCurves.Remove(curve.Key);
        }
        ...
        private void BlendOutPlayable()
        {
            ...
            if (_activeIndex > _layerLevel)
                for (var i = _layerLevel; i < _activeIndex; i++)
                {
                    var cache = _playables[i - _layerLevel].cachedWeight;
                    var inactiveWeight = Mathf.Lerp(cache, 0f, BlendOutWeight);
