...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.AdsLayer
{
    public class AdsLayerState : WeaponLayerState
    {
        ...
        {
            var result = rotation.eulerAngles;
        ...
        {
            var result = KTransform.Identity;
        ...
        {
            var isAiming = _inputController.GetValue<bool>(_isAimingPropertyIndex);
        ...
        protected override void OnEvaluateWeaponPose()
        {
            ...
            if (_settings.cameraBlend > 0f) _aimTargetBone.localPosition = _targetDefaultPose;
            ...
            var weight = KCurves.Ease(0f, 1f, _aimingWeight, _settings.aimingEaseMode) * Weight;

            var blend = _settings.positionBlend;
            var pose = GetAdsPose();
            ...
            var absQ = GetEuler(pose.rotation);
            var addQ = GetEuler(_additivePose.rotation);
            ...
            var aimPoint = GetLocalAimPoint(_entity.defaultAimPoint);
            _aimPoint = KTransform.EaseLerp(_prevAimPoint, aimPoint, _aimPointPlayback, _settings.aimPointEaseMode);

            var socketRotation = _aimPoint.rotation;
            var socketPosition = _aimPoint.position;
