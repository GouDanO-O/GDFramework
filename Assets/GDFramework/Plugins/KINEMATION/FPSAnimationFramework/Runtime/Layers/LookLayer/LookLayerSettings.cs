...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.LookLayer
{
    ...
    public class LookLayerSettings : FPSAnimatorLayerSettings
    {
        ...
        public List<LookLayerElement> pitchOffsetElements = new();
        public List<LookLayerElement> yawOffsetElements = new();
        public List<LookLayerElement> rollOffsetElements = new();
        ...
        private void ApplyAngleDistribution(ref List<LookLayerElement> collection)
        {
            ...
            var angleToDistribute = Vector2.zero;

            var bShallDistribute = false;
            var bDistributeForX = false;
            ...
            if (bShallDistribute)
                for (var i = adjustStartIndex; i < count; i++)
                {
                    var element = collection[i];
