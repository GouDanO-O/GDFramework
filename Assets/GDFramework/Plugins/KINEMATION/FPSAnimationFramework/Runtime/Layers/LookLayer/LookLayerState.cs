...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.LookLayer
{
    public class LookLayerState : FPSAnimatorLayerState
    {
        ...
        {
            var turnInput = _inputController.GetValue<float>(_turnProperty);
            _lookInput = _inputController.GetValue<Vector4>(_mouseInputPropertyIndex);

            if (_settings.useTurnOffset) _lookInput.x = turnInput;

            var lean = _inputController.GetValue<float>(_leanPropertyIndex);

            var fraction = lean / 90f;
            var sign = fraction > 0f;

            foreach (var element in _settings.rollOffsetElements)
            {
                var angle = sign ? element.clampedAngle.x : element.clampedAngle.y;
            ...
            var spaceRotation = _owner.transform.rotation * Quaternion.Euler(0f, _lookInput.x, 0f);

            foreach (var element in _settings.pitchOffsetElements)
            {
                var angle = sign ? element.clampedAngle.x : element.clampedAngle.y;
                var bone = _rigComponent.GetRigTransform(element.rigElement);
