...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.AttachHandLayer
{
    public class AttachHandLayerState : FPSAnimatorLayerState
    {
        ...
        private void RefreshSettings(FPSAnimatorLayerSettings newSettings)
        {
            ...
            var hasValidCustomPose = _settings.customHandPose != null;
            ...
            if (hasValidCustomPose) _rigComponent.ApplyHierarchyCachedPose();
        }
        ...
        {
            var attachedPose = new KTransform();
        ...
        {
            var handlePos = _ikHandBone.position;
            var handleRot = _ikHandBone.rotation;

            if (Tools.current == Tool.Move)
            {
                handlePos = Handles.PositionHandle(handlePos, _ikWeaponBone.rotation);

                var localHandle = _ikWeaponBone.InverseTransformPoint(handlePos);
                var localHand = _ikWeaponBone.InverseTransformPoint(_ikHandBone.position);
            ...
            else if (Tools.current == Tool.Rotate)
            {
                ...
                var weaponInverse = Quaternion.Inverse(_ikWeaponBone.rotation);
                var localHandle = weaponInverse * handleRot;
