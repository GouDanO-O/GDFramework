...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.SwayLayer
{
    ...
    public class SwayLayerState : WeaponLayerState
    {
        ...
        {
            var useFreeAim = _inputController.GetValue<bool>(_freeAimPropertyIndex);
            ...
            var rotation = Quaternion.Euler(new Vector3(_freeAimValue.y, _freeAimValue.x, 0f));
            rotation.Normalize();

            var headMS = _owner.transform.InverseTransformPoint(_headBone.position);
            var masterMS = _owner.transform.InverseTransformPoint(_weaponIkBone.position);

            var offset = headMS - masterMS;
            offset = rotation * offset - offset;

            var freeAimTransform = new KTransform()
            {
                position = -offset,
                rotation = rotation
            };
        ...
        {
            var moveInput = _inputController.GetValue<Vector4>(_moveInputPropertyIndex);
            ...
            var alpha = KMath.ExpDecayAlpha(_settings.moveSwayTargetDamping, Time.deltaTime);
        ...
        {
            var deltaRight = _mouseDelta.x;
            var deltaUp = _mouseDelta.y;

            _aimSwayTarget += new Vector2(deltaRight, deltaUp) * 0.01f;

            var alpha = KMath.ExpDecayAlpha(_settings.aimSwayTargetDamping, Time.deltaTime);
            ...
            var aimSwayTransform = new KTransform()
            {
