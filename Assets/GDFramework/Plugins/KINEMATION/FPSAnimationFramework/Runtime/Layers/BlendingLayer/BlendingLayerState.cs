...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.BlendingLayer
{
    public class BlendingLayerState : FPSAnimatorLayerState
    {
        ...
        public override void InitializeState(FPSAnimatorLayerSettings newSettings)
        {
            ...
            {
                var transform = _rigComponent.GetRigTransform(element.elementToBlend);
        ...
        public override void OnEvaluatePose()
        {
            ...
            var count = _elementTransforms.Count;
            for (var i = 0; i < count; i++) _basePose.Add(new KTransform(_elementTransforms[i]));

            for (var i = 0; i < count; i++)
            {
                var transform = _elementTransforms[i];
                var source = _basePose[i];
                var target = new KTransform(_owner.transform).GetWorldTransform(_cachedPose[i], false);
                var weight = Weight * _settings.blendingElements[i].weight;

                if (_settings.blendPosition)
                    transform.position = Vector3.Lerp(source.position, target.position, weight);
