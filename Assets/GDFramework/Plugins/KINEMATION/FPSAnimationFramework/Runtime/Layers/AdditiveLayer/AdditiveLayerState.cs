...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.AdditiveLayer
{
    ...
    public class AdditiveLayerState : WeaponLayerState
    {
        ...
        {
            var weight = Weight;
            var aimingWeight = _inputController.GetValue<float>(_aimingWeightPropertyIndex);

            var pivotOffset = Vector3.zero;
            var recoilR = Quaternion.identity;
            var recoilT = Vector3.zero;
            ...
            var t = KMath.ExpDecayAlpha(_settings.interpSpeed, Time.deltaTime);
            if (Mathf.Approximately(_settings.interpSpeed, 0f)) t = 1f;
