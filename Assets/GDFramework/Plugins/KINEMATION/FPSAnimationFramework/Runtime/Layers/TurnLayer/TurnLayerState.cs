...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.TurnLayer
{
    public class TurnLayerState : FPSAnimatorLayerState
    {
        ...
        {
            var mouseDelta = _inputController.GetValue<Vector4>(_mouseDeltaProperty).x;
            ...
            if (_isTurning)
            {
                ...
                var alpha = _settings.turnCurve.Evaluate(_playBack);
        ...
        {
            var offset = Quaternion.Euler(0f, _turnAngle, 0f);
            KAnimationMath.RotateInSpace(_owner.transform, _characterRootBone, offset, 1f);

            var localPosition = _characterRootBone.localPosition;
