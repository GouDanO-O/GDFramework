...
namespace KINEMATION.FPSAnimationFramework.Runtime.Layers.ViewLayer
{
    public class ViewLayerState : FPSAnimatorLayerState
    {
        ...
        {
            var component = _owner.transform;
        ...
        {
            var space = _ikGunBone.transform;

            if (_settings.ikHandGun.space == ESpaceType.ParentBoneSpace) space = _ikGunBone.parent;

            if (_settings.ikHandGun.space == ESpaceType.ComponentSpace) space = _owner.transform;

            if (_settings.ikHandGun.space == ESpaceType.WorldSpace) space = null;

            var spaceRot = space == null ? Quaternion.identity : space.rotation;
            var handlePos = Handles.PositionHandle(_ikGunBone.position, spaceRot);
            ...
            var boneLocal = space.InverseTransformPoint(_ikGunBone.position);
