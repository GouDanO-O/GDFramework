...
namespace KINEMATION.FPSAnimationFramework.Runtime.Core
{
    ...
    public class FPSAnimatorProfile : ScriptableObject, IRigObserver
    {
        ...
        public List<FPSAnimatorLayerSettings> settings = new();

        public bool Equals(FPSAnimatorProfile anotherProfile)
        {
            if (anotherProfile == null || settings.Count != anotherProfile.settings.Count) return false;
        ...
        private void OnValidate()
        {
            ...
            if (rigAsset == _cachedRigAsset) return;

            if (_cachedRigAsset != null) _cachedRigAsset.UnRegisterObserver(this);
