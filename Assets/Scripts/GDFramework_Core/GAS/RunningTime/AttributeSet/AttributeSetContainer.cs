...
namespace GDFramework_Core.GAS.RunningTime.AttributeSet
{
    public class AttributeSetContainer
    {
        ...
        private readonly Dictionary<string, AttributeSet> _attributeSetDict = new Dictionary<string, AttributeSet>();

        private readonly Dictionary<AttributeBase, AttributeAggregator> _attributeAggregatorDict =
        ...
        public void RemoveAttributeSet<T>() where T : AttributeSet
        {
            ...
            foreach (var attr in attrSet.AttributeNames)
            {
                _attributeAggregatorDict.Remove(attrSet[attr]);
            }
